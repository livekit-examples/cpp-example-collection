...
bool SDLMicSource::init() {
  ...
  if (!SDL_ResumeAudioStreamDevice(stream_)) { // unpause device
    std::cerr << "[error] Failed to resume recording device: " << SDL_GetError() << "\n";
    return false;
  }
...
bool DDLSpeakerSink::init() {
  ...
  if (!stream_) {
    std::cerr << "[error] Failed to open playback stream: " << SDL_GetError() << "\n";
    return false;
  }

  if (!SDL_ResumeAudioStreamDevice(stream_)) {
    std::cerr << "[error] Failed to resume playback device: " << SDL_GetError() << "\n";
    return false;
  }
...
void DDLSpeakerSink::enqueue(const int16_t* samples, int num_samples_per_channel) {
  ...
  if (!SDL_PutAudioStreamData(stream_, samples, bytes)) {
    std::cerr << "[error] SDL_PutAudioStreamData failed: " << SDL_GetError() << "\n";
  }
}
...
bool SDLCamSource::init() {
  ...
  if (!camera_) {
    std::cerr << "[error] Failed to open camera: " << SDL_GetError() << "\n";
    return false;
  }
