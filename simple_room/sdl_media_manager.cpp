...
bool SDLMediaManager::ensureSDLInit(Uint32 flags) {
  ...
  if (!SDL_InitSubSystem(flags)) {
    std::cerr << "[error] SDL_InitSubSystem failed (flags=" << flags << "): " << SDL_GetError() << "\n";
    return false;
  }
  return true;
}
...
bool SDLMediaManager::startMic(const std::shared_ptr<AudioSource>& audio_source) {
  ...
  if (!audio_source) {
    std::cerr << "[error] startMic: audioSource is null\n";
    return false;
  }
...
bool SDLMediaManager::startCamera(const std::shared_ptr<VideoSource>& video_source) {
  ...
  if (!video_source) {
    std::cerr << "[error] startCamera: videoSource is null\n";
    return false;
  }
...
bool SDLMediaManager::startSpeaker(const std::shared_ptr<AudioStream>& audio_stream) {
  ...
  if (!audio_stream) {
    std::cerr << "[error] startSpeaker: audioStream is null\n";
    return false;
  }

  if (!ensureSDLInit(SDL_INIT_AUDIO)) {
    std::cerr << "[error] startSpeaker: SDL_INIT_AUDIO failed\n";
    return false;
  }
...
void SDLMediaManager::speakerLoopSDL() {
  ...
  while (speaker_running_.load(std::memory_order_relaxed)) {
    ...
    if (!localStream) {
      ...
      if (dev == 0) {
        std::cerr << "[error] speakerLoopSDL: SDL_GetAudioStreamDevice failed: " << SDL_GetError() << "\n";
        break;
      }

      if (!SDL_ResumeAudioDevice(dev)) {
        std::cerr << "[error] speakerLoopSDL: SDL_ResumeAudioDevice failed: " << SDL_GetError() << "\n";
        break;
      }
    }
    ...
    if (!SDL_PutAudioStreamData(localStream, data.data(), numBytes)) {
      std::cerr << "[error] speakerLoopSDL: SDL_PutAudioStreamData failed: " << SDL_GetError() << "\n";
      break;
    }
...
bool SDLMediaManager::initRenderer(const std::shared_ptr<VideoStream>& video_stream) {
  ...
  if (!ensureSDLInit(SDL_INIT_VIDEO)) {
    std::cerr << "[error] startRenderer: SDL_INIT_VIDEO failed\n";
    return false;
  }
  ...
  if (!sdl_renderer_) {
    ...
    if (!sdl_renderer_->init("LiveKit Remote Video", 1280, 720)) {
      std::cerr << "[error] startRenderer: SDLVideoRenderer::init failed\n";
      sdl_renderer_.reset();
