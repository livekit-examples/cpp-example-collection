...
public:
  ...
  void onTrackSubscribed(livekit::Room& /*room*/, const livekit::TrackSubscribedEvent& ev) override {
    ...
    if (ev.track && ev.track->kind() == TrackKind::KIND_VIDEO) {
      ...
      if (!video_stream) {
        std::cerr << "[error] Failed to create VideoStream for track " << track_sid << "\n";
        return;
      }

      MainThreadDispatcher::dispatch([this, video_stream] {
        if (!media_.initRenderer(video_stream)) {
          std::cerr << "[error] SDLMediaManager::startRenderer failed for track\n";
        }
      });
      ...
      MainThreadDispatcher::dispatch([this, audio_stream] {
        if (!media_.startSpeaker(audio_stream)) {
          std::cerr << "[error] SDLMediaManager::startSpeaker failed for track\n";
        }
      });
    }
  }
...
int main(int argc, char* argv[]) {
  ...
  if (!SDL_Init(SDL_INIT_VIDEO)) {
    std::cerr << "[error] SDL_Init(SDL_INIT_VIDEO) failed: " << SDL_GetError() << "\n";
