...
int main(int argc, char *argv[]) {
  ...
  std::cout << "[info] ping connected as identity='" << local_participant->identity()
  ...
  if (!publish_result) {
    ...
    std::cerr << "[error] Failed to publish ping data track: code="
      ...
      [&sent_messages,
       &sent_messages_mutex](const std::vector<std::uint8_t> &payload,
        ...
        try {
          ...
          std::cout << "[info] pong id=" << metrics.id
  ...
  std::cout << "[info] published data track '" << ping_pong::kPingTrackName
            << "' and listening for '" << ping_pong::kPongTrackName << "' from '"
  ...
  while (g_running.load()) {
    ...
    if (!push_result) {
      ...
      std::cerr << "[warn] Failed to push ping data frame: code="
                << static_cast<std::uint32_t>(error.code) << " message=" << error.message << "\n";
    } else {
      {
        std::lock_guard<std::mutex> lock(sent_messages_mutex);
